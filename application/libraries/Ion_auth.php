...
class Ion_auth
{
	...
	public function forgotten_password($identity)    //changed $email to $identity
	{
		if ( $this->ion_auth_model->forgotten_password($identity) )   //changed
		{
            ...
            $user = $this->where($this->config->item('identity', 'ion_auth'), $identity)->where('active', 1)->users_auth()->row();  //changed to get_user_by_identity from email
	...
	public function in_group($check_group, $id=false, $check_all = false)
	{
		...
		{
			$users_auth_groups = $this->ion_auth_model->get_users_auth_groups($id)->result();
			$groups_array = array();
			foreach ($users_auth_groups as $group)
			{
