...
class CI_Cizacl
{
	...
	function check_cizacl()
	{
		...
		$resource = $this->uri->segment(1) ? $this->uri->segment(1) : $this->router->routes['default_controller'];
	...
	function css()
	{
		...
		$data .=	'<!--[if lte IE 8]>'.PHP_EOL.
					'<link href="'.base_url().'assets/cizacl/css/cizaclIE.css" rel="stylesheet" type="text/css" />'.PHP_EOL.
					'<![endif]-->'.PHP_EOL;
		return $data;
	}
	...
	function scripts()
	{
		$array = array(
			...
			'jquery.colorbox.min',
			'i18n/jqgrid/grid.locale-'.$this->cizacl_mdl->getAbbr('assets/cizacl/js/i18n/jqgrid/','grid.locale-xx.js'),
			'jquery.jqGrid.min',
		);
		$data = '';
		foreach($array as $value)	{
