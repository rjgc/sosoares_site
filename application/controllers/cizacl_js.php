...
class Cizacl_Js extends CI_Controller {
	...
	public function scripts()
	{
		...
		function make_menu()	{
			...
			item1.append($("<a/>", {"href":"'.site_url('cizacl/users').'", "text":"'.$this->lang->line('users').'"}));
			item2.append($("<a/>", {"href":"'.site_url('cizacl/management').'", "text":"'.$this->lang->line('roles_resources_rules').'"}));
			item3.append($("<a/>", {"href":"'.site_url('cizacl/sessions').'", "text":"'.$this->lang->line('sessions').'"}));
			item4.append($("<a/>", {"href":"'.site_url('cizacl').'", "text":"'.$this->lang->line('summary').'"}));
	...
	public function users()
	{
		...
		$(document).ready(function()	{
			$("'.$jqgrid_name.'").jqGrid({
				url: "'.site_url('cizacl/users_load_data').'",
				datatype: "json",
				mtype: "POST",
				...
				sortorder: "asc",
				viewrecords: true,
				editurl: "'.site_url('cizacl/users_op').'",
				pager: "'.$navgrid_name.'"
			})
		...
		function view()	{
			...
			if(id != null)
				colorbox("/cizacl/user_view/"+row.'.$id_field_name.',400,400);
			else
				alert("'.$this->lang->line('select_row').'");
		}

		function add()	{
			colorbox("/cizacl/user_add/",440,400,null,"reloadGrid");
		}
		...
		function edit()	{
			...
			if(id != null)
				colorbox("/cizacl/user_edit/"+row.'.$id_field_name.',440,400,null,"reloadGrid");
			else
				alert("'.$this->lang->line('select_row').'");
		}
	...
	public function sessions()
	{
		...
		$(document).ready(function()	{
			$("'.$jqgrid_name.'").jqGrid({
				url: "'.site_url('cizacl/sessions_load_data').'",
				datatype: "json",
				mtype: "POST",
				...
				viewrecords: true,
				multiselect: true,
				editurl: "'.site_url('cizacl/sessions_op').'",
				pager: "'.$navgrid_name.'"
			})
	...
	public function cizacl()
	{
				...
				show: function(event, ui) {
					if(ui.index == 0 && !jqgrid_initialized[0]){
						$("'.$jqgrid_name[0].'").jqGrid({
							url: "'.site_url('cizacl/roles_load_data').'",
							datatype: "json",
							mtype: "POST",
						...
						$("'.$jqgrid_name[1].'").jqGrid({
							url: "'.site_url('cizacl/resources_load_data').'",
							datatype: "json",
							mtype: "POST",
						...
						$("'.$jqgrid_name[2].'").jqGrid({
							url: "'.site_url('cizacl/rules_load_data').'",
							datatype: "json",
							mtype: "POST",
		...
		function add_resource()	{
			colorbox("/cizacl/resource_add/",450,350,null,"reloadGrid");
		}
		...
		function edit_resource()	{
			...
			if(id != null)
				colorbox("/cizacl/resource_edit/"+row.'.$id_field_name[1].',450,350,null,"reloadGrid");
			else
				alert("'.$this->lang->line('select_row').'");
		}
		...
		function add_rule()	{
			colorbox("/cizacl/rule_add/","850","450",null,"reloadGrid");
		}
		...
		function edit_rule()	{
			...
			if(id != null)
				colorbox("/cizacl/rule_edit/"+row.'.$id_field_name[2].',"850","450",null,"reloadGrid");
			else
				alert("'.$this->lang->line('select_row').'");
		}
	...
	public function resource_oper()
	{
		...
		$(document).ready(function()	{
			$("#form1").submit(function()	{
				...
				$.ajax({
					url:		"'.site_url('cizacl/resources_op').'",
					type:		"post",
					data:		$(this).serialize(),
					dataType:	"json",
	...
	public function rule_oper()
	{
		...
		function getControllers()	{
			$.ajax({
				url:		"'.site_url("cizacl/getControllers").'",
				type:		"post",
				dataType:	"json",
				success:	function(data)	{
		...
		function getFunctions()	{
			$.ajax({
				url:		"'.site_url("cizacl/getFunctions").'",
				type:		"post",
				dataType:	"json",
				success:	function(data)	{
					if(data.response == true)	{
		...
		function getRules()	{
			$.ajax({
				url:		"'.site_url("cizacl/getRules").'/"+$("#cizacl_role_id").val(),
				type:		"post",
				dataType:	"json",
				success:	function(data)	{
