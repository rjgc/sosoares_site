...
class Ion_auth_mongodb_model extends CI_Model {
	...
	public function hash_password_db($id, $password, $use_sha1_override = FALSE)
	{
		...
		$document = $this->mongo_db
			...
			->limit(1)
			->get($this->collections['users_auth']);
	...
	public function activate($id, $code = FALSE)
	{
		...
		if ($code !== FALSE)
		{
			...
			$updated = $this->mongo_db
				...
				->update($this->collections['users_auth']);
		}
	...
	public function username_check($username = '')
	{
		...
		return count($this->mongo_db
			...
			->get($this->collections['users_auth'])) > 0;
	}
	...
	public function forgotten_password_complete($code, $salt=FALSE)
	{
		...
		$profile = $this->where('forgotten_password_code', $code)->users_auth()->document();
		...
		// But if there were no users_auth with that forgotten password code:
		$this->trigger_events(array(
			'post_forgotten_password_complete',
			'post_forgotten_password_complete_unsuccessful'
		));
	...
	/**
	 * Inserts a user document into users_auth collection.
	 *
	 * @return bool
	 */
	public function register($username, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		$data = array_merge($this->_filter_data($this->collections['users_auth'], $additional_data), $data);

		$this->trigger_events('extra_set');

		// Insert new document and store the _id value
		$id = $this->mongo_db->insert($this->collections['users_auth'], $data);
	...
	public function users_auth($groups = NULL)
	{
		...
		$this->response = $this->mongo_db->get($this->collections['users_auth']);
		return $this;
	}
	...
	public function update($id, array $data)
	{
		...
		$data = $this->_filter_data($this->collections['users_auth'], $data);
	...
	public function delete_user($id)
	{
		...
		$deleted = $this->mongo_db
			->where('_id', new MongoId($id))
			->delete($this->collections['users_auth']);
	...
	**/
	...
	protected function _filter_data($collection, $data)
	{
		...
		$columns = $collection == 'users_auth' ?
			// users_auth collection static schema array
