...
class Ion_auth_model extends CI_Model
{
	...
	/**
	 * caching of users_auth and their groups
	 *
	 * @var array
	 **/
	 ...
	 * against an entry in the users_auth table.
	 ...
	 * Deactivae : Updates a users_auth row with an activation code.
	 *
	 * @author Mathew
	 */
	...
	public function activate($id, $code = false)
	{
		...
		if ($code !== FALSE)
		{
			...
			$this->trigger_events('extra_where');
			$this->db->update($this->tables['users_auth'], $data, array($this->identity_column => $identity));
		}
		else
		{
			$data = array(
			...
			$this->trigger_events('extra_where');
			$this->db->update($this->tables['users_auth'], $data, array('id' => $id));
		}
	...
	public function clear_forgotten_password_code($code) {
		...
		if ($this->db->count_all_results($this->tables['users_auth']) > 0)
		{
			$data = array(
			...
			$this->db->update($this->tables['users_auth'], $data, array('forgotten_password_code' => $code));
	...
	public function change_password($identity, $old, $new)
	{
		...
		if ($old_password_matches === TRUE)
		{
			...
			$successfully_changed_password_in_db = $this->db->update($this->tables['users_auth'], $data, array($this->identity_column => $identity));
	...
	public function username_check($username = '')
	{
		...
		return $this->db->where('username', $username)
		                ->count_all_results($this->tables['users_auth']) > 0;
	}
	...
	public function forgotten_password($identity)
	{
		...
		$this->db->update($this->tables['users_auth'], $update, array($this->identity_column => $identity));
	...
	public function forgotten_password_complete($code, $salt=FALSE)
	{
		...
		$profile = $this->where('forgotten_password_code', $code)->users_auth()->row(); //pass the code to profile
	...
	public function register($username, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		//filter out any data passed that doesnt have a matching column in the users_auth table
		//and merge the set user data and the additional data
		$user_data = array_merge($this->_filter_data($this->tables['users_auth'], $additional_data), $data);

		$this->trigger_events('extra_set');

		$this->db->insert($this->tables['users_auth'], $user_data);
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($password === TRUE)
			{
				...
				if ($remember && $this->config->item('remember_users_auth', 'ion_auth'))
				{
	 ...
	 * users_auth
	 *
	 * @return object users_auth
	 * @author Ben Edmunds
	 **/
	public function users_auth($groups = NULL)
	{
		$this->trigger_events('users_auth');
		...
		else
		{
			...
			$this->db->select(array(
			    $this->tables['users_auth'].'.*',
			    $this->tables['users_auth'].'.id as id',
			    $this->tables['users_auth'].'.id as user_id'
			));
		}
		...
		if (isset($groups))
		{
			...
			if (isset($groups) && !empty($groups))
			{
				$this->db->distinct();
				$this->db->join(
				    $this->tables['users_auth_groups'],
				    $this->tables['users_auth_groups'].'.'.$this->join['users_auth'].'='.$this->tables['users_auth'].'.id',
				    'inner'
				);

				$this->db->where_in($this->tables['users_auth_groups'].'.'.$this->join['groups'], $groups);
			}
		}
		...
		$this->response = $this->db->get($this->tables['users_auth']);
	...
	public function user($id = NULL)
	{
		...
		//if no id was passed use the current users_auth id
		$id || $id = $this->session->userdata('user_id');

		$this->limit(1);
		$this->where($this->tables['users_auth'].'.id', $id);
	...
	/**
	 * get_users_auth_groups
	 *
	 * @return array
	 * @author Ben Edmunds
	 **/
	public function get_users_auth_groups($id=FALSE)
	{
		$this->trigger_events('get_users_auth_group');

		//if no id was passed use the current users_auth id
		$id || $id = $this->session->userdata('user_id');

		return $this->db->select($this->tables['users_auth_groups'].'.'.$this->join['groups'].' as id, '.$this->tables['groups'].'.name, '.$this->tables['groups'].'.description')
		                ->where($this->tables['users_auth_groups'].'.'.$this->join['users_auth'], $id)
		                ->join($this->tables['groups'], $this->tables['users_auth_groups'].'.'.$this->join['groups'].'='.$this->tables['groups'].'.id')
		                ->get($this->tables['users_auth_groups']);
	}
	...
	public function add_to_group($group_id, $user_id=false)
	{
		...
		if ($this->db->where(array( $this->join['groups'] => (int)$group_id, $this->join['users_auth'] => (int)$user_id))->get($this->tables['users_auth_groups'])->num_rows()) return false;

		if ($return = $this->db->insert($this->tables['users_auth_groups'], array( $this->join['groups'] => (int)$group_id, $this->join['users_auth'] => (int)$user_id)))
		{
	...
	public function remove_from_group($group_ids=false, $user_id=false)
	{
		...
		if( ! empty($group_ids))
		{
			...
			{
				$this->db->delete($this->tables['users_auth_groups'], array($this->join['groups'] => (int)$group_id, $this->join['users_auth'] => (int)$user_id));
		...
		{
			if ($return = $this->db->delete($this->tables['users_auth_groups'], array($this->join['users_auth'] => (int)$user_id))) {
	...
	public function update($id, array $data)
	{
		...
		$data = $this->_filter_data($this->tables['users_auth'], $data);
		...
		$this->trigger_events('extra_where');
		$this->db->update($this->tables['users_auth'], $data, array('id' => $user->id));
	...
	**/
	public function delete_user($id)
	{
		...
		// delete user from users_auth table should be placed after remove from group
		$this->db->delete($this->tables['users_auth'], array('id' => $id));
	...
	public function update_last_login($id)
	{
		...
		$this->db->update($this->tables['users_auth'], array('last_login' => time()), array('id' => $id));
	...
	public function remember_user($id)
	{
		...
		$this->db->update($this->tables['users_auth'], array('remember_code' => $salt), array('id' => $id));
	...
	public function login_remembered_user()
	{
		...
		if ($query->num_rows() == 1)
		{
			...
			//extend the users_auth cookies if the option is enabled
	...
	**/
	public function delete_group($group_id = FALSE)
	{
		...
		// remove all users_auth from this group
		$this->db->delete($this->tables['users_auth_groups'], array($this->join['groups'] => $group_id));
